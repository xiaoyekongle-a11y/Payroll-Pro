module.exports = async function handler(req, res) {
  ...
  const prompt = `あなたは給与計算ルールのパーサーです。
指示文を解析してJSON配列のみを返してください。説明文・マークダウン・コードブロック不要。

指示: ${instruction}

【ルールタイプ】
- perf_multiply : 業績係数を掛け算する。「N倍」「N割増」はこれ。value は倍率（例: 2倍→2.0, 1.2倍→1.2）
- perf_set      : 業績係数を固定値にする。value は係数値（例: 係数1.5固定→1.5）
- allowance_add : 手当金額を加算する。value は円（例: 5000円→5000）
...
【重要な解釈ルール】
- 「給与をN倍」「給料N倍」「全員N倍」→ perf_multiply, target:"all", value:N
- 「N割増」→ perf_multiply, value:1+N/10（例:2割増→1.2）
- 「N%アップ」→ perf_multiply, value:1+N/100
- 「N円の手当」「N円加算」→ allowance_add, value:N
- 「係数をNにする」→ perf_set, value:N
- 解釈できない場合は空配列 []

【出力例】
指示「給与全員2倍」→ [{"type":"perf_multiply","target":"all","value":2.0,"label":"全員×2倍"}]
指示「営業部1.2倍、全員に5000円手当」→ [{"type":"perf_multiply","target":"dept:営業部","value":1.2,"label":"営業部×1.2倍"},{"type":"allowance_add","target":"all","value":5000,"label":"全員手当+5000円"}]
    ...
    const groqRes = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a Japanese payroll rule parser. Return ONLY a valid JSON array. No explanation, no markdown, no code blocks.',
          },
